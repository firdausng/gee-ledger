<script lang="ts">
	...
	let { items = $bindable<ServiceItem[]>([]) } = $props();

	let dragIndex = $state<number | null>(null);

	function rowAmount(item: ServiceItem): number {
		const rate = parseFloat(item.rate) || 0;
		return rate * item.hours;
	}

	const grandTotal = $derived(items.reduce((sum, i) => sum + rowAmount(i), 0));

	function addItem() {
		items = [...items, { description: '', hours: 1, rate: '0.00' }];
	}
		...
		<table class="w-full text-sm">
			<thead>
				<tr class="border-b border-border bg-muted/50">
					...
					<th class="text-right px-2 py-2 text-xs font-medium text-muted-foreground w-16">Hours</th>
					<th class="text-right px-2 py-2 text-xs font-medium text-muted-foreground w-24">Rate/hr</th>

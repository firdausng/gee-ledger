...
<div class="bg-white text-gray-900 font-sans {compact ? 'p-4 sm:p-6' : 'p-10'}">
	...
	<!-- Business header -->
	<div class="flex items-start gap-3 pb-4 border-b border-gray-200 mb-5 {compact ? 'gap-2.5' : 'gap-4'}">
		{#if biz.logoR2Key}
			<img
				src="/api/businesses/{businessId}/logo"
				alt="{biz.name} logo"
				class="{compact ? 'h-8 w-8' : 'h-14 w-14'} object-contain rounded shrink-0"
			/>
		{/if}
	...
	<!-- Document title -->
	<p class="{compact ? 'text-lg' : 'text-3xl'} font-bold tracking-wide text-gray-900 mb-4">{title}</p>

	<!-- Meta + Bill To: stack on compact, side-by-side on full -->
	<div class="{compact ? 'flex flex-col gap-4' : 'grid grid-cols-2 gap-6'} mb-5 {compact ? 'text-[11px]' : 'text-sm'}">
		<!-- Invoice meta -->
		<div class="flex flex-col gap-0.5 text-gray-600">
			...
			{:else if documentType !== 'receipt' && invoiceNo}
				<div class="flex gap-2">
					<span class="text-gray-400 {compact ? 'w-18' : 'w-24'} shrink-0">Invoice No</span>
					<span class="font-semibold">{invoiceNo}</span>
				</div>
			{/if}
			<div class="flex gap-2">
				<span class="text-gray-400 {compact ? 'w-18' : 'w-24'} shrink-0">Date</span>
				<span>{tx.transactionDate}</span>
			</div>
			{#if tx.referenceNo}
				<div class="flex gap-2">
					<span class="text-gray-400 {compact ? 'w-18' : 'w-24'} shrink-0">Reference</span>
					<span>{tx.referenceNo}</span>
				</div>
			{/if}
			{#if location}
				<div class="flex gap-2">
					<span class="text-gray-400 {compact ? 'w-18' : 'w-24'} shrink-0">Location</span>
					<span>{location.name}</span>
				</div>
			{/if}
			{#if category}
				<div class="flex gap-2">
					<span class="text-gray-400 {compact ? 'w-18' : 'w-24'} shrink-0">Category</span>
		...
		{#if hasBillTo}
			<div>
				<p class="{compact ? 'text-[10px]' : 'text-xs'} font-semibold text-gray-400 uppercase tracking-wider mb-1">Bill To</p>
				{#if billTo?.name}
	...
	<!-- Note / description -->
	{#if tx.note}
		<div class="mb-5 {compact ? 'text-[11px]' : 'text-sm'} text-gray-700 border-t border-gray-100 pt-3">
			<p class="{compact ? 'text-[10px]' : 'text-xs'} font-semibold text-gray-400 uppercase tracking-wider mb-1">Description</p>
			<p>{tx.note}</p>
		</div>
	{/if}
	...
	{#if hasItems}
		<div class="mb-5 border border-gray-200 rounded-lg overflow-hidden {compact ? 'text-[11px]' : 'text-sm'}">
			<table class="w-full">
				<thead>
					<tr class="bg-gray-50 border-b border-gray-200">
						<th class="text-left {compact ? 'px-2 py-2' : 'px-4 py-2.5'} font-semibold text-gray-600">Description</th>
						<th class="text-right {compact ? 'px-1.5 py-2' : 'px-4 py-2.5'} font-semibold text-gray-600 {compact ? 'w-8' : 'w-12'}">Qty</th>
						<th class="text-right {compact ? 'px-1.5 py-2' : 'px-4 py-2.5'} font-semibold text-gray-600 {compact ? 'w-20' : 'w-28'}">Price</th>
						<th class="text-right {compact ? 'px-2 py-2' : 'px-4 py-2.5'} font-semibold text-gray-600 {compact ? 'w-20' : 'w-28'}">Amount</th>
					</tr>
				</thead>
				<tbody>
					{#each items as item}
	...
	{:else}
		<!-- Flat amount box -->
		<div class="bg-gray-50 rounded-lg {compact ? 'p-3' : 'p-5'} text-right mb-5 border border-gray-200">
			<p class="{compact ? 'text-[11px]' : 'text-sm'} text-gray-500 mb-1">Total Amount</p>
			<p class="{compact ? 'text-xl' : 'text-4xl'} font-bold text-gray-900">{amount}</p>
		</div>
	{/if}

...
export async function assignInvoiceNoHandler(
	user: App.User,
	businessId: string,
	...
	if (tx.invoiceNo) return { invoiceNo: tx.invoiceNo };
	...
	const n = biz.nextInvoiceNo;
	const invoiceNo = `INV-${String(n).padStart(4, '0')}`;

	await db
		.update(transactions)
		.set({ invoiceNo, updatedAt: now, updatedBy: user.id })
		.where(eq(transactions.id, transactionId));

	await db
		.update(businesses)
		.set({ nextInvoiceNo: n + 1, updatedAt: now, updatedBy: user.id })

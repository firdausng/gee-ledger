...
import { getCategoryHandler } from './getCategoryHandler';
...
export const categoriesApi = new Hono<App.Api>()
	...
	.get('/businesses/:businessId/categories/:categoryId', async (c) => {
		const user = c.get('currentUser');
		const data = await getCategoryHandler(user, c.req.param('businessId'), c.req.param('categoryId'), c.env);
		return c.json({ data });
	})
